...
- (void)sd_setBackgroundImageWithURL:(NSURL *)url forState:(UIControlState)state placeholderImage:(UIImage *)placeholder options:(SDWebImageOptions)options completed:(SDWebImageCompletionBlock)completedBlock {
    ...
    if (url) {
        ...
        NSString *headImgURL = url.absoluteString;
        //不是微信绑定登陆的
        if (![headImgURL hasPrefix:@"http://wx"]) {
            //不是完整的URL就拼接一下
            if ([headImgURL hasPrefix:@"/images"]) {
                headImgURL = [@"http://www.winservices.cn" stringByAppendingString:headImgURL];
            }
        }
        //删除URL里的空格
        headImgURL = [headImgURL stringByTrimmingCharactersInSet:[NSCharacterSet characterSetWithCharactersInString:@" "]];
        url = [NSURL URLWithString:headImgURL];
