...
- (void)viewDidLoad
{
    ...
    self.view.backgroundColor = [UIColor clearColor];//kPhotoBrowserBackgrounColor;
    [self addScrollView];
    [self addToolbars];
    [self setUpFrames];
    _initialIndex = self.currentImageIndex;
}
...
- (void)showPhotoBrowser
{
    ...
    if (self.sourceImagesContainerView == nil) {
        rect = self.imageFrameinWebView;
        
    }
    else {
        UIView *sourceView = self.sourceImagesContainerView.subviews[self.currentImageIndex];
        UIView *parentView = [self getParsentView:sourceView];
        rect = [sourceView.superview convertRect:sourceView.frame toView:parentView];
...
- (void)hidePhotoBrowser:(UITapGestureRecognizer *)recognizer
{
    ...
    if (self.sourceImagesContainerView == nil) {
        targetTemp = self.imageFrameinWebView;
    }
    else {
        UIView *sourceView = self.sourceImagesContainerView.subviews[self.currentImageIndex];
        UIView *parentView = [self getParsentView:sourceView];
    ...
    [UIView animateWithDuration:kPhotoBrowserHideDuration animations:^{
        if (_initialIndex == self.currentImageIndex) {
            tempImageView.frame = targetTemp;
